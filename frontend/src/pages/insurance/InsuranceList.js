...
const InsuranceList = () => {
  ...
  const handleAdd = async (e) => {
    ...
    try {
      await createInsuranceAPI({ name: newName.trim() });
      setNewName('');
  ...
  const handleUpdate = async (id) => {
    ...
    try {
      await updateInsuranceAPI(id, { name: editName.trim() });
      setEditId(null);
        ...
        <table className="w-full">
          ...
          <tbody className="divide-y divide-gray-100">
            {loading ? (
              ...
              <tr><td colSpan={3} className="py-8 text-center text-gray-400">No insurance companies added yet</td></tr>
            ) : items.map((item, idx) => (
