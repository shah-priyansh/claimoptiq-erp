...
const TPAList = () => {
  ...
  const handleBulkApply = async (statusAutomation) => {
    ...
    const ok = await confirm(
      willClear
        ? `Clear status automation on ${selectedIds.length} selected TPA(s)?`
        : `Apply this status automation to ${selectedIds.length} selected TPA(s)? This replaces their existing rules.`,
      { title: 'Apply Status Automation', confirmLabel: willClear ? 'Clear' : 'Apply', variant: willClear ? 'danger' : 'primary' },
    );
    if (!ok) return;
    setApplying(true);
    try {
      const { data } = await bulkTPAStatusAutomationAPI(selectedIds, statusAutomation);
