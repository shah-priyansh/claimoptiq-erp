...
- [ ] **Step 3: Hand-author the migration + apply with `migrate deploy`**

This repo's migrations are hand-authored SQL applied via `prisma migrate deploy` (NOT `migrate dev`, which rebuilds a shadow DB by replaying all historical migrations — that replay fails here on an out-of-order legacy migration). Create `backend/prisma/migrations/20260812120000_journal_entries/migration.sql` with exactly:
...
Then run (from `backend/`): `npx prisma migrate deploy` followed by `npx prisma generate`.
Expected: `migrate deploy` reports the one new migration applied; `generate` regenerates the client with the new models. If `migrate deploy` reports drift/failed, STOP and report — do not reset.
...
Run (from `backend/`): `node scripts/smokeJournal.js` — must run from backend/ so dotenv finds backend/.env
